...
use crate::{
    ...
    IriRef,
    IriRefBuf,
    common::{RiBufImpl, RiImpl, RiRefBufImpl, RiRefImpl, bytestr_eq, parse},
};
...
#[grammar(file = "src/uri/grammar.abnf", entry_point = "URI", ascii, cache = "automata/uri.aut.cbor")]
...
impl Uri {
    ...
    #[inline]
    pub fn suffix(&self, prefix: &(impl ?Sized + AsRef<Uri>)) -> Option<(PathBuf, Option<&Query>, Option<&Fragment>)> {
