...
use crate::{
    ...
    Uri,
    UriBuf,
    common::{RiRefBufImpl, RiRefImpl, parse},
};

use super::{Authority, AuthorityMut, Fragment, Path, PathBuf, PathMut, Query, Scheme, bytestr_eq};
...
impl UriRef {
    ...
    #[inline]
    pub fn suffix(&self, prefix: &(impl ?Sized + AsRef<UriRef>)) -> Option<(PathBuf, Option<&Query>, Option<&Fragment>)> {
...
mod tests {
    ...
    const PARTS: [(&[u8], (Option<&[u8]>, Option<&[u8]>, &[u8], Option<&[u8]>, Option<&[u8]>)); 36] = [
        // 0 components.
        ...
        (b"scheme:?query", (Some(b"scheme"), None, b"", Some(b"query"), None)),
        // 2 components.
        (b"scheme://authority", (Some(b"scheme"), Some(b"authority"), b"", None, None)),
        (b"scheme:path", (Some(b"scheme"), None, b"path", None, None)),
        (b"scheme:/path", (Some(b"scheme"), None, b"/path", None, None)),
        (b"scheme:?query", (Some(b"scheme"), None, b"", Some(b"query"), None)),
        (b"scheme:#fragment", (Some(b"scheme"), None, b"", None, Some(b"fragment"))),
        (b"//authority/path", (None, Some(b"authority"), b"/path", None, None)),
        (b"//authority?query", (None, Some(b"authority"), b"", Some(b"query"), None)),
        (b"//authority#fragment", (None, Some(b"authority"), b"", None, Some(b"fragment"))),
        (b"path?query", (None, None, b"path", Some(b"query"), None)),
        (b"/path?query", (None, None, b"/path", Some(b"query"), None)),
        (b"path#fragment", (None, None, b"path", None, Some(b"fragment"))),
        (b"?query#fragment", (None, None, b"", Some(b"query"), Some(b"fragment"))),
        // 3 components
        (b"scheme://authority/path", (Some(b"scheme"), Some(b"authority"), b"/path", None, None)),
        (b"scheme://authority?query", (Some(b"scheme"), Some(b"authority"), b"", Some(b"query"), None)),
        (
            b"scheme://authority#fragment",
            (Some(b"scheme"), Some(b"authority"), b"", None, Some(b"fragment")),
        ),
        (b"scheme:path?query", (Some(b"scheme"), None, b"path", Some(b"query"), None)),
        (b"scheme:path#fragment", (Some(b"scheme"), None, b"path", None, Some(b"fragment"))),
        (b"//authority/path?query", (None, Some(b"authority"), b"/path", Some(b"query"), None)),
        (b"//authority/path#fragment", (None, Some(b"authority"), b"/path", None, Some(b"fragment"))),
        (
            b"//authority?query#fragment",
            (None, Some(b"authority"), b"", Some(b"query"), Some(b"fragment")),
        ),
        (b"path?query#fragment", (None, None, b"path", Some(b"query"), Some(b"fragment"))),
        // 4 components
        (
            b"scheme://authority/path?query",
            (Some(b"scheme"), Some(b"authority"), b"/path", Some(b"query"), None),
        ),
        (
            b"scheme://authority/path#fragment",
            (Some(b"scheme"), Some(b"authority"), b"/path", None, Some(b"fragment")),
        ),
        (
            b"scheme://authority?query#fragment",
            (Some(b"scheme"), Some(b"authority"), b"", Some(b"query"), Some(b"fragment")),
        ),
        (
            b"scheme:path?query#fragment",
            (Some(b"scheme"), None, b"path", Some(b"query"), Some(b"fragment")),
        ),
        // 5 components
        (
            b"scheme://authority/path?query#fragment",
            (Some(b"scheme"), Some(b"authority"), b"/path", Some(b"query"), Some(b"fragment")),
        ),
    ];
