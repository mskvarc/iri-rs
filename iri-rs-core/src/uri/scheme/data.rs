...
impl DataUrl {
    ...
    pub fn decoded_data(&self) -> Result<Cow<[u8]>, base64::DecodeError> {
        ...
        if self.is_base_64_encoded() {
            base64::engine::general_purpose::STANDARD.decode(encoded).map(Cow::Owned)
        } else {
...
impl DataUrlBuf {
    ...
    pub fn from_string(url: String) -> Result<Self, InvalidDataUrl> {
        Self::new(url).map_err(|InvalidDataUrl(bytes)| InvalidDataUrl(unsafe { String::from_utf8_unchecked(bytes) }))
    }
...
impl<'de> serde::Deserialize<'de> for &'de DataUrl {
    ...
    {
        <&'de str>::deserialize(deserializer)?.try_into().map_err(serde::de::Error::custom)
    }
}
