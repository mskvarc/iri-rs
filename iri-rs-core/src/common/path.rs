...
pub trait PathImpl: 'static {
    ...
    fn last(&self) -> Option<&Self::Segment> {
        if self.is_empty() {
            ...
            unsafe { self.previous_segment_from(self.as_bytes().len() + 1).map(|(segment, _)| segment) }
        }
    }
    ...
    fn parent_or_empty(&self) -> &Self {
        self.parent()
            .unwrap_or_else(|| if self.is_absolute() { Self::EMPTY_ABSOLUTE } else { Self::EMPTY })
    }
...
pub enum SegmentsImpl<'a, P: ?Sized> {
    Empty,
    NonEmpty { path: &'a P, offset: usize, back_offset: usize },
}
...
impl<'a, P: ?Sized + PathImpl> Iterator for SegmentsImpl<'a, P> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        match self {
            Self::Empty => None,
            Self::NonEmpty { path, offset, back_offset } => {
...
impl<'a, P: ?Sized + PathImpl> DoubleEndedIterator for SegmentsImpl<'a, P> {
    fn next_back(&mut self) -> Option<Self::Item> {
        match self {
            ...
            SegmentsImpl::NonEmpty { path, offset, back_offset } => {
                if offset < back_offset {
    ...
    fn new(path: &'a P) -> NormalizedSegmentsImpl<'a, P> {
        ...
        for segment in path.segments() {
            match segment.as_bytes() {
                CURRENT_SEGMENT => (),
                PARENT_SEGMENT => {
                    if stack.last().map(|s| s.as_bytes() == PARENT_SEGMENT).unwrap_or(relative) {
                        stack.push(segment)
                    } else {
                        stack.pop();
                    }
