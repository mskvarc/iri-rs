...
use super::{
    parse,
    path::{CURRENT_SEGMENT, PARENT_SEGMENT, PathBufImpl, PathImpl, SegmentImpl},
};
...
impl<'a, P: ?Sized + PathImpl> PathMutImpl<'a, P> {
    ...
    fn first_segment_offset(&self) -> usize {
        if self.is_absolute() { self.start + 1 } else { self.start }
    }
    ...
    pub fn push(&mut self, segment: &P::Segment) {
        ...
        let disambiguate = self.is_empty() && ((self.start == 0 && segment.looks_like_scheme()) || segment.is_empty());
    ...
    pub fn pop(&mut self) -> bool {
        ...
        if (is_empty && self.is_relative()) || self.last().map(SegmentImpl::as_bytes) == Some(PARENT_SEGMENT) {
