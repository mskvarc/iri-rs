...
use super::{AuthorityImpl, AuthorityMutImpl, FragmentImpl, PathImpl, PathMutImpl, QueryImpl, RiBufImpl, RiImpl, SegmentImpl, parse};
...
pub trait RiRefImpl {
    ...
    type RiRefBuf: Default + RiRefBufImpl<Authority = Self::Authority, Path = Self::Path, Query = Self::Query, Fragment = Self::Fragment>;
    ...
    fn authority(&self) -> Option<&Self::Authority> {
        let bytes = self.as_bytes();
        parse::find_authority(bytes, 0)
            .ok()
            .map(|range| unsafe { <Self::Authority as AuthorityImpl>::new_unchecked(&bytes[range]) })
    }
    ...
    fn relative_to(&self, other: &Self) -> Self::RiRefBuf {
        ...
        match (self.scheme_opt(), other.scheme_opt()) {
            ...
            (None, Some(_)) => (),
            (None, None) => (),
            _ => return unsafe { <Self::RiRefBuf as RiRefBufImpl>::new_unchecked(self.as_bytes().to_vec()) },
        }
        ...
        let mut base_segments = other.path().parent_or_empty().normalized_segments().peekable();
        ...
        for _segment in base_segments {
            result.path_mut().push(<<Self::Path as PathImpl>::Segment as SegmentImpl>::PARENT);
        }
        ...
        if (self.query().is_some() || self.fragment().is_some()) && Some(result.path().as_bytes()) == other.path().last().map(|s| s.as_bytes()) {
...
pub trait RiRefBufImpl: Sized + RiRefImpl {
    type Ri: ?Sized + RiImpl<Authority = Self::Authority, Path = Self::Path, Query = Self::Query, Fragment = Self::Fragment>;

    type RiBuf: ?Sized + RiBufImpl + RiImpl<Authority = Self::Authority, Path = Self::Path, Query = Self::Query, Fragment = Self::Fragment>;
