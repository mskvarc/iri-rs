...
use crate::{
    ...
    UriRef,
    UriRefBuf,
    common::{RiRefBufImpl, RiRefImpl, parse, str_eq},
    uri::InvalidUriRef,
};
...
impl IriRef {
    pub fn parts(&self) -> IriRefParts<'_> {
        ...
        IriRefParts {
            scheme: ranges.scheme.map(|r| unsafe { Scheme::new_unchecked(&bytes[r]) }),
            authority: ranges.authority.map(|r| unsafe { Authority::new_unchecked(&self.0[r]) }),
            path: unsafe { Path::new_unchecked(&self.0[ranges.path]) },
            query: ranges.query.map(|r| unsafe { Query::new_unchecked(&self.0[r]) }),
            fragment: ranges.fragment.map(|r| unsafe { Fragment::new_unchecked(&self.0[r]) }),
        }
    }
    ...
    #[inline]
    pub fn suffix(&self, prefix: &(impl ?Sized + AsRef<IriRef>)) -> Option<(PathBuf, Option<&Query>, Option<&Fragment>)> {
...
impl IriRefBuf {
    ...
    pub fn from_vec(buffer: Vec<u8>) -> Result<Self, InvalidIriRef<Vec<u8>>> {
        match String::from_utf8(buffer) {
            Ok(string) => Self::new(string).map_err(|InvalidIriRef(s)| InvalidIriRef(s.into_bytes())),
    ...
    pub fn try_into_uri(self) -> Result<UriBuf, InvalidUri<Self>> {
        UriBuf::new(self.into_bytes()).map_err(|InvalidUri(bytes)| unsafe { InvalidUri(Self::new_unchecked(String::from_utf8_unchecked(bytes))) })
    }

    /// Converts this IRI reference into an URI reference, if possible.
    pub fn try_into_uri_ref(self) -> Result<UriRefBuf, InvalidUriRef<Self>> {
        UriRefBuf::new(self.into_bytes()).map_err(|InvalidUriRef(bytes)| unsafe { InvalidUriRef(Self::new_unchecked(String::from_utf8_unchecked(bytes))) })
    }
...
mod tests {
    ...
    const PARTS: [(&str, (Option<&str>, Option<&str>, &str, Option<&str>, Option<&str>)); 36] = [
        // 0 components.
        ("", (None, None, "", None, None)),
        // 1 component.
        ...
        ("scheme:?query", (Some("scheme"), None, "", Some("query"), None)),
        // 2 components.
        ("scheme://authority", (Some("scheme"), Some("authority"), "", None, None)),
        ("scheme:path", (Some("scheme"), None, "path", None, None)),
        ("scheme:/path", (Some("scheme"), None, "/path", None, None)),
        ("scheme:?query", (Some("scheme"), None, "", Some("query"), None)),
        ("scheme:#fragment", (Some("scheme"), None, "", None, Some("fragment"))),
        ("//authority/path", (None, Some("authority"), "/path", None, None)),
        ("//authority?query", (None, Some("authority"), "", Some("query"), None)),
        ("//authority#fragment", (None, Some("authority"), "", None, Some("fragment"))),
        ("path?query", (None, None, "path", Some("query"), None)),
        ("/path?query", (None, None, "/path", Some("query"), None)),
        ("path#fragment", (None, None, "path", None, Some("fragment"))),
        ("?query#fragment", (None, None, "", Some("query"), Some("fragment"))),
        // 3 components
        ("scheme://authority/path", (Some("scheme"), Some("authority"), "/path", None, None)),
        ("scheme://authority?query", (Some("scheme"), Some("authority"), "", Some("query"), None)),
        ("scheme://authority#fragment", (Some("scheme"), Some("authority"), "", None, Some("fragment"))),
        ("scheme:path?query", (Some("scheme"), None, "path", Some("query"), None)),
        ("scheme:path#fragment", (Some("scheme"), None, "path", None, Some("fragment"))),
        ("//authority/path?query", (None, Some("authority"), "/path", Some("query"), None)),
        ("//authority/path#fragment", (None, Some("authority"), "/path", None, Some("fragment"))),
        ("//authority?query#fragment", (None, Some("authority"), "", Some("query"), Some("fragment"))),
        ("path?query#fragment", (None, None, "path", Some("query"), Some("fragment"))),
        // 4 components
        (
            "scheme://authority/path?query",
            (Some("scheme"), Some("authority"), "/path", Some("query"), None),
        ),
        (
            "scheme://authority/path#fragment",
            (Some("scheme"), Some("authority"), "/path", None, Some("fragment")),
        ),
        (
            "scheme://authority?query#fragment",
            (Some("scheme"), Some("authority"), "", Some("query"), Some("fragment")),
        ),
        ("scheme:path?query#fragment", (Some("scheme"), None, "path", Some("query"), Some("fragment"))),
        // 5 components
        (
            "scheme://authority/path?query#fragment",
            (Some("scheme"), Some("authority"), "/path", Some("query"), Some("fragment")),
        ),
    ];
    ...
    fn more_resolutions1() {
        ...
        let tests = [("#s", "http://a/bb/ccc/d;p?q#s"), ("", "http://a/bb/ccc/d;p?q")];
    ...
    fn more_resolutions3() {
        ...
        let tests = [("xyz", "http://ab//de//xyz"), ("./xyz", "http://ab//de//xyz"), ("../xyz", "http://ab//de/xyz")];
