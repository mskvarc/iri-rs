...
use iri_rs_core::iri::{Authority, Fragment, Path, Query, Segment};
use iri_rs_core::uri::Scheme;
...
fn bench_setters(c: &mut Criterion) {
    ...
    c.bench_function("mutate/set_scheme", |b| {
        b.iter_batched_ref(
            seed,
            |buf| buf.set_scheme(Some(black_box(new_scheme))),
    ...
    c.bench_function("mutate/set_authority_replace", |b| {
        b.iter_batched_ref(
            seed,
            |buf| buf.set_authority(Some(black_box(new_authority))),
            BatchSize::SmallInput,
        );
    });
    ...
    c.bench_function("mutate/set_path_longer", |b| {
        b.iter_batched_ref(
            seed,
            |buf| buf.set_path(black_box(new_path_buf)),
            BatchSize::SmallInput,
        );
    });
    ...
    c.bench_function("mutate/set_query_replace", |b| {
        b.iter_batched_ref(
            seed,
            |buf| buf.set_query(Some(black_box(new_query))),
            BatchSize::SmallInput,
        );
    });
    ...
    c.bench_function("mutate/set_fragment_replace", |b| {
        b.iter_batched_ref(
            seed,
            |buf| buf.set_fragment(Some(black_box(new_fragment))),
            BatchSize::SmallInput,
        );
    });
...
fn bench_path_mut(c: &mut Criterion) {
    ...
    c.bench_function("mutate/path_mut_push", |b| {
        b.iter_batched_ref(
            seed,
            |buf| buf.path_mut().push(black_box(seg)),
            BatchSize::SmallInput,
        );
    });
    ...
    c.bench_function("mutate/path_mut_symbolic_push_dotdot", |b| {
        b.iter_batched_ref(
            seed,
            |buf| buf.path_mut().symbolic_push(black_box(dotdot)),
            BatchSize::SmallInput,
        );
    });
}
