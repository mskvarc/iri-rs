...
//!
//! Because the generated code references `::iri_rs_core::*` paths, downstream
//! crates using the derive must have `iri-rs-core` as a direct dependency.
...
fn positions_tokens(p: Positions) -> TokenStream2 {
    let s = p.scheme_end;
    let a = p.authority_end;
    let pe = p.path_end;
    let q = p.query_end;
    quote! {
        ::iri_rs_core::Positions {
            scheme_end: #s,
            authority_end: #a,
            path_end: #pe,
            query_end: #q,
...
fn iri_const_tokens(iri: &IriBuf) -> TokenStream2 {
    let s = iri.as_str();
...
pub fn iri_enum_derive(input: TokenStream) -> TokenStream {
    ...
    for variant in data.variants {
        ...
        match variant.fields {
            Fields::Unit => {
                ...
                let raw = iri.as_str();
                let iri_expr = iri_const_tokens(&iri);
            ...
            Fields::Unnamed(fields) if fields.unnamed.len() == 1 => {
                ...
                try_from_default = quote! {
                    match <#ty as ::core::convert::TryFrom<&::iri_rs_core::Iri<__T>>>::try_from(iri) {
                        ::core::result::Result::Ok(value) => ::core::result::Result::Ok(#type_id::#variant_ident(value)),
                        ::core::result::Result::Err(_) => { #try_from_default }
                    }
                };
                into_arms.extend(quote! {
                    #type_id::#variant_ident(v) => <&#ty as ::core::convert::Into<::iri_rs_core::Iri<&'static str>>>::into(v),
                });
            }
            Fields::Named(_) => bail!(
                variant_ident.span(),
    ...
    let output = quote! {
        impl<__T> ::core::convert::TryFrom<&::iri_rs_core::Iri<__T>> for #type_id
        where
            __T: ::core::ops::Deref<Target = str>,
        {
            ...
            #[inline]
            fn try_from(iri: &::iri_rs_core::Iri<__T>) -> ::core::result::Result<Self, ()> {
                match iri.as_str() {
                    #try_from_arms
                    _ => #try_from_default,
                }
        ...
        impl ::core::convert::From<&#type_id> for ::iri_rs_core::Iri<&'static str> {
            #[inline]
        ...
        impl ::core::convert::From<#type_id> for ::iri_rs_core::Iri<&'static str> {
            #[inline]
            fn from(value: #type_id) -> Self {
                <&#type_id as ::core::convert::Into<::iri_rs_core::Iri<&'static str>>>::into(&value)
            }
        }
    };
